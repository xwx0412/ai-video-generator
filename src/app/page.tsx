...
import { Play, Clock, CheckCircle, XCircle, Trash2, ImageIcon } from "lucide-react";
        ...
        <div className="flex-1 flex ml-[220px] transition-all duration-300">
          {activeTab === "generator" && (
            <>
              ...
              {/* Right: Preview Panel */}
              <div className="w-[400px] flex-shrink-0 flex flex-col overflow-hidden bg-black/20">
                ...
                <div className="border-t border-white/5">
                  <div className="p-3 border-b border-white/5">
                    <h4 className="text-gray-400 text-xs font-medium">Recent</h4>
                  </div>
                  <div className="max-h-[200px] overflow-y-auto p-2 space-y-1">
                    {history.length === 0 ? (
                      <p className="text-gray-600 text-xs text-center py-4">No videos yet</p>
                    ) : (
                      history.slice(0, 5).map((record) => (
                        <div
                          key={record.id}
                          onClick={() => setResult({ videoUrl: record.videoUrl, prompt: record.prompt })}
                        ...
                        >
                          <div className="w-12 h-8 rounded bg-black/50 overflow-hidden flex-shrink-0">
                            <video src={record.videoUrl} className="w-full h-full object-cover" preload="metadata" />
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-white text-xs truncate">{record.prompt || "Image"}</p>
                    ...
                    {history.map((record) => (
                      <div
                        ...
                        onClick={() => {
                          ...
                          setResult({ videoUrl: record.videoUrl, prompt: record.prompt });
                        }}
